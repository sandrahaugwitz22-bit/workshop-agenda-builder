...
const validateAgenda = (parsed, totalMinutes) => {
  ...
  // Edge case 3: allow ±5 min tolerance
...
const generateAgenda = async (form) => {
  ...
  return withRetry(async () => {
    // Edge case 4: network timeout via AbortController (30s)
    ...
    try {
      ...
      // Edge case 4b: request aborted (timeout)
    ...
    // Edge case 2: strip markdown fences before parsing
    ...
    // Edge case 3: ±5 min tolerance on duration mismatch
...
const ActionBar = ({ agenda, onEdit, onRegenerate }) => {
  ...
  return (
    <div className="flex flex-wrap gap-3 mt-6 print:hidden">
      <button
        onClick={handleCopy}
        className="flex-1 min-w-[120px] bg-white border border-gray-200 hover:border-indigo-400 text-gray-700 text-sm font-medium py-2.5 rounded-xl transition-all"
      >
        {copied ? "✅ Copied!" : "📋 Copy agenda"}
      </button>
      <button
        onClick={() => downloadTxt(agenda)}
        className="flex-1 min-w-[120px] bg-white border border-gray-200 hover:border-indigo-400 text-gray-700 text-sm font-medium py-2.5 rounded-xl transition-all"
      >
        ⬇️ Download .txt
      </button>
      ...
      <button
        onClick={onEdit}
        className="flex-1 min-w-[120px] bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold py-2.5 rounded-xl transition-all"
      >
        ✏️ Edit inputs
      </button>
    </div>
...
const EmptyState = ({ onEdit }) => (
  <div className="text-center py-16">
    ...
    <button
      onClick={onEdit}
      className="bg-indigo-600 text-white text-sm font-semibold px-6 py-2.5 rounded-xl hover:bg-indigo-700 transition-all"
    >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 flex flex-col items-center justify-center gap-6">
      ...
      <div className="text-center">
        <p className="text-gray-800 font-semibold text-lg transition-all">{LOADING_MESSAGES[msgIndex]}</p>
...
const ErrorBanner = ({ message, onRetry }) => (
  <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 flex items-center justify-center px-4">
    <div className="bg-white border border-rose-200 rounded-2xl p-8 max-w-md w-full text-center shadow-sm">
      ...
      <button
        onClick={onRetry}
        className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-6 py-2.5 rounded-xl text-sm transition-all"
      >
        Try again
      </button>
    </div>
...
export default function App() {
  const [view, setView] = useState("form"); // "form" | "loading" | "error" | "result"
  ...
  const handleSubmit = async (form) => {
    ...
    try {
      ...
      setAgenda(result);
      setView("result");
      console.log("✅ Agenda JSON:", result);
    } catch (err) {
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1 text-sm">
            Fill in the details below and we'll generate a full agenda for you in seconds.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 flex flex-col gap-6">
            ...
            <FormField label="Workshop Goal / Objective" required error={errors.goal}>
              <textarea
                rows={3}
                placeholder="e.g. Understand our users' main pain points around onboarding"
                value={form.goal}
                onChange={(e) => set("goal")(e.target.value)}
                className={`w-full px-3 py-2 rounded-lg border text-sm resize-none focus:outline-none focus:ring-2 focus:ring-indigo-400 ${
                  errors.goal ? "border-rose-400" : "border-gray-300"
                }`}
              />
            </FormField>
            ...
            <FormField label="Target Audience / Participants" required error={errors.audience}>
              <input
                type="text"
                placeholder="e.g. UX designers and product managers"
                value={form.audience}
                onChange={(e) => set("audience")(e.target.value)}
                className={`w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 ${
                  errors.audience ? "border-rose-400" : "border-gray-300"
                }`}
              />
            </FormField>
            ...
            <FormField label="Number of Participants" required error={errors.participantCount}>
              <input
                ...
                className={`w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 ${
                  errors.participantCount ? "border-rose-400" : "border-gray-300"
                }`}
              />
            </FormField>
              ...
              <div className="flex flex-wrap gap-2">
                {WORKSHOP_TYPES.map((t) => (
                  <button
                    key={t.value}
                    type="button"
                    onClick={() => set("workshopType")(t.value)}
                    className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${
                      form.workshopType === t.value
                        ? "bg-indigo-600 text-white border-indigo-600"
                        : "bg-white text-gray-600 border-gray-300 hover:border-indigo-400"
                    }`}
                  >
            ...
            <button
              type="submit"
              className="w-full bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white font-semibold py-3 rounded-xl transition-all text-sm shadow-md shadow-indigo-100"
            >
              ✨ Generate Agenda
            </button>
          </div>
        ...
        <p className="text-center text-xs text-gray-400 mt-4">
          Your inputs stay private. Nothing is stored.
        </p>
      </div>
    </div>
  );
